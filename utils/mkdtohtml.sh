...
sed  -e "1,3s/<!---title:\(.*\)-->/<title>\1<\/title>/p" $src > temp_head_keywords.html # 生成<head>中的标题标签
sed  -e "1,3s/<!---keywords:\(.*\)-->/<meta name=\"keywords\" content=\"\1\">/p" $src >> temp_head_keywords.html # 生成<head>中的关键字标签
sed  -e "1,3s/<!---title:\(.*\)-->/<h1>\1<\/h1>/p" $src > temp_title.html # 生成<body>中的标题标签<h1>
...
sed  -e "1,3s/<!---date:\(.*\)-->/<p>date: \1<\/p>/p" $src > temp_title.html # 生成<body>中的标题标签<p>时间
